 ...
 * }
 */
export function isMarketOpen(market, testDate) {
  if (!market) {
    return {
      status: "CLOSED",
      code: "closed",
 ...
 */
export function getNextOpenTime(market, testDate) {
  if (!market) return "Schedule pending";
 ...
 */
export function getCurrentSeason(testDate) {
  const now = testDate ? new Date(testDate) : new Date();
...
/**
 * Safely prepends leading slash to local image paths for Vite
 */
export function getAssetUrl(path) {
  if (!path) return "/images/markets/market-1.jpg";
